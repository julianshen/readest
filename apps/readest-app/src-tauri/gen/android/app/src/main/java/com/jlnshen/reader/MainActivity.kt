...
class MainActivity : TauriActivity(), KeyDownInterceptor {
    ...
    // Throttle touch event forwarding to at most ~30fps
    private val lastTouchEventMs = AtomicLong(0L)
    private companion object {
        private const val TOUCH_THROTTLE_MS = 33L
    }

    private fun isEinkDevice(): Boolean {
        // Quick check using known Boox system properties
        return try {
            val propClass = Class.forName("android.os.SystemProperties")
            val get = propClass.getMethod("get", String::class.java)
            val manufacturer = (get.invoke(null, "ro.product.manufacturer") as? String)?.lowercase() ?: ""
            val model = (get.invoke(null, "ro.product.model") as? String)?.lowercase() ?: ""
            val einkSupport = (get.invoke(null, "ro.eink.support") as? String) ?: ""

            manufacturer.contains("onyx") || manufacturer.contains("boox") ||
            model.contains("eink") || model.contains("e-ink") ||
            einkSupport.isNotEmpty()
        } catch (e: Exception) {
            false
        }
    }
    ...
    override fun onWebViewCreate(webView: WebView) {
        wv = webView
        if (isEinkDevice()) {
            // Use software layer for e-ink to avoid HW composition flicker
    ...
    /**
     * Force a WebView relayout when the window gains focus.
     *
     * Fixes a race condition (#3297) on Android 14+ where the WebView is
     * created and its URL loaded AFTER onWindowFocusChanged(true) has already
     * fired. The window never repaints the content, leaving a blank screen.
     * Triggering invalidate() on focus forces the compositor to re-draw.
     */
    ...
    private val keyEventMap = mapOf(
        ...
        // Boox e-ink device-specific key codes
        KeyEvent.KEYCODE_PAGE_UP to "PageUp",    // 92
        KeyEvent.KEYCODE_PAGE_DOWN to "PageDown" // 93
    )
    ...
    override fun dispatchTouchEvent(event: MotionEvent): Boolean {
        ...
        // Throttle only MOVE floods; DOWN/UP/CANCEL must always reach JS or
        // a fast tap (<33ms) loses its touchend and leaves gestures hanging.
        if (event.actionMasked == MotionEvent.ACTION_MOVE &&
            now - lastTouchEventMs.get() < TOUCH_THROTTLE_MS
        ) {
            return super.dispatchTouchEvent(event)
        }
        lastTouchEventMs.set(now)
